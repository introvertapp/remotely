...
grep -q 'protocol-core-device-persistence.patch' "$SCRIPT_DIR/build.sh" || fail "successful-device persistence patch integration missing"
[[ -f Patches/protocol-core-device-persistence.patch ]] || fail "successful-device persistence patch missing"
grep -q '^-        LastConnectedDeviceStorage.save(device)' Patches/protocol-core-device-persistence.patch || fail "early last-device persistence removal missing"
grep -q '^+                        LastConnectedDeviceStorage.save(device)' Patches/protocol-core-device-persistence.patch || fail "successful-session device persistence addition missing"
grep -q 'private var startupReconnectDeviceID: String?' Sources/remotely/AppleTVService.swift || fail "app-owned startup reconnect target missing"
grep -q 'let persistedSelectedDeviceID = defaults.string(forKey: DefaultsKey.selectedID)' Sources/remotely/AppleTVService.swift || fail "persisted app device ID is not captured for startup reconnect"
grep -q 'let device = map\[startupReconnectDeviceID\]' Sources/remotely/AppleTVService.swift || fail "startup reconnect does not resolve the app-persisted device"
! grep -q 'connectToLastConnectedDevice()' Sources/remotely/AppleTVService.swift || fail "startup still trusts the core's independently persisted last-device record"
[[ "$(grep -c 'startupReconnectDeviceID = nil' Sources/remotely/AppleTVService.swift)" -ge 4 ]] || fail "startup reconnect target is not consumed/cancelled safely"
